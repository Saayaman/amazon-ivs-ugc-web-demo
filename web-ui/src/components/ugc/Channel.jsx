...
class Channel extends Component {
  ...
  getAndSetStreamInfo = (username) => {
    this.getCurrentStreamInfo(username).then((currentStream) => {
  ...
  tick() {
    ...
    if (config.USE_MOCK_DATA) {
      ...
      Object.keys(currentStream.channelStatus).length
    ) {
  ...
  render() {
    ...
    const { userInfo, signedIn, checkedAuth } = this.props;
    ...
    if (config.USE_MOCK_DATA) {
      ...
      videoStream = Object.keys(currentStream.channel).length
        ? currentStream.channel.channel.playbackUrl
        : "";
    }

    // Check if stream is live
    const isLive =
      streamId && currentStream ? currentStream.isLive === "Yes" : false;
    ...
    if (!isMyChannel && userInfoValid) {
      if (
        userInfo.preferred_username === currentStream.username ||
        this.props.channelUser === userInfo.preferred_username
      ) {
        isMyChannel = true;
      }
    }
            ...
            <fieldset>
              <div className="stream-title-field">
                <input
                  ...
                  onKeyDown={(e) =>
                    this.handleStreamTitleKeyDown(e, currentStreamTitle)
                  }
                ...
                <button
                  ...
                  onClick={this.handleMyStreamTitleClick}
                >
                  Save
                </button>
              </div>
        ...
        <div className="main stream-container">
          <div className="content-wrapper mg-2">
            ...
            <div className="stream-title mg-t-1">
              {titleComponent}
              <div className="channel-meta pd-t-1">{userMetaComponent}</div>
            </div>
          </div>
        </div>
      </React.Fragment>
